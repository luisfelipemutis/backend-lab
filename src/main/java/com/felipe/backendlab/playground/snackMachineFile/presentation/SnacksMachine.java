...
public class SnacksMachine {
    ...
    private static void maquinaSnacks() {
        ...
        serviceSnacks.showSnacks(); // Mostrar snaks disponibles.

        while (!exit) {
            try {
                var option = showMenu(sc);
                exit = executeOptions(option, sc, products, serviceSnacks);
            } catch (Exception e) {
    ...
    private static boolean executeOptions(int option, Scanner sc, List<Snack> products, IServiceSnacks serviceSnacks) {
        boolean exit = false;
        switch (option) {
            case 1 -> buySnack(sc, products, serviceSnacks);
            case 2 -> showTicket(products);
            case 3 -> addNewSnack(sc, serviceSnacks);
    ...
    private static void addNewSnack(Scanner sc, IServiceSnacks serviceSnacks) {
        System.out.print("Nombre del snack: ");
        var nameSnack = sc.nextLine();
        System.out.print("Precio del snack:");
        var price = Double.parseDouble(sc.nextLine());
        serviceSnacks.addSnacks(new Snack(nameSnack, price));
